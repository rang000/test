...
#include <stdlib.h>
#include <string.h>
#define NEW(p,n)  malloc(sizeof(p)*n);
...
void _dfs1(Graph *g, int *flag, int v, int *stack, int *sp){
  int j;
  Edge *e;
  flag[v] = 1;  // 頂点 v の全ての辺に対して、その辺の先に訪れていないなら再帰呼び出し。
...
int scc(Graph *g, int *component){
  int i;
  int *flag = NEW(int,g->n)   // 探索済みかどうかのフラグ
  int *stack = NEW(int,g->n) // 初回深さ優先探索の終了順に頂点を格納する場所
  int sp = 0, c = 0;
  Graph *rg;
  for(i = 0; i < g->n; i++){
     flag[i] = 0; // フラグクリア
...
void pagerank(Graph *g, double *rank){
  int * ls = NEW(int,g->n)  // 出ていくリンク数
  double *new_rs = NEW(double,g->n)  // 新しいランク用の配列
  Edge *e ;
